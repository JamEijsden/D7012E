...
value (Num n) _ = n
value (Add p q) dict = (value p dict) + (value q dict)  
value (Sub p q) dict = (value p dict) - (value q dict)  
value (Mul p q) dict = (value p dict) * (value q dict)
value (Div p q) dict
	|	value q dict /= 0 = quot (value p dict) (value q dict)
	|	otherwise  = error "Expr.value: division by 0"
value (Var v) dict =
